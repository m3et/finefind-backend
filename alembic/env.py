...
# ... etc.
POSTGRES_DATABASE_URL = str(sqlalchemy_settings.POSTGRES_DATABASE_URL)

db_driver = sqlalchemy_settings.POSTGRES_DATABASE_URL.scheme
db_driver_parts = db_driver.split("+")
if len(db_driver_parts) > 1:  # e.g. postgresql+asyncpg
    sync_scheme = db_driver_parts[0].strip()
    POSTGRES_DATABASE_URL = POSTGRES_DATABASE_URL.replace(  # replace with sync driver
